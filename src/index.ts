...
class Kelp extends Object {
  ...
  constructor(set: KelpOptions) {
        ...
        if (stat.isDirectory()) {
          ...
          if (!routeData.method)
            throw new InvalidRouteError(
              `Route ${routePath} does not have a method.`
            );
          if (!this.isValidMethod(routeData.method))
            throw new InvalidRouteError(
              `Route ${routePath} has an invalid method.`
            );
          ...
          this.log(
            `${this.colorSec("Using method")} ${this.colorPrim(
              routeData.method
            )} ${this.colorSec("route")} ${this.colorPrim(
              routeData.path
            )} in ${this.colorPrim(routePath.replace(dirname__, ""))}}`
          );
        }
      });
    };
    ...
    const dirname__ = __dirname.replace("node_modules/@znci/kelp/out", "");
    if (set.PORT) {
      if (!this.validatePort(String(set.PORT)))
        throw new InvalidPortError(`${set.PORT} is not a valid port.`);
      PORT = set.PORT;
    }
    if (set.OPTIONS) {
      if (set.OPTIONS.cors) {
      ...
      if (set.OPTIONS.public) {
        app.use(
          express.static(path.join(dirname__, set.OPTIONS.publicOptions!.path!))
        );
      }
